...
r_PRF_pop_1 = zeros(nScans,1);
r_PRF_pop_2 = zeros(nScans,1);
r_PRF_pop_3 = zeros(nScans,1);
...
parfor sc = 1:nScans
    ...
    r_PRF_pop_1(sc) = corr(GS, yPred);
    yPred_card = regr(:,2)*B(2);  r_PRF_pop_2(sc) = corr(yPred_card,GS);
    yPred_resp = regr(:,3)*B(3);  r_PRF_pop_3(sc) = corr(yPred_resp,GS);
    
end
r_PRF_pop = [r_PRF_pop_1,r_PRF_pop_2,r_PRF_pop_3];
obj_function =1 - mean(r_PRF_pop_1);
